...
async def generate_text_summary_via_url_or_text(body_flag, body, request: Request, db: Session = Depends(get_db),
                                                summary_type="factual"):
    try:
        record_audit(data={"site_body": body}, event_type=f'Inside generate_text_summary_via_url_or_text Method',
                     log_type='info', db=db, request=request)
        ...
        if body_flag == 1:
            ...
            if summary_db_obj is not None:
                if summary_db_obj.response_text != "":
                    return 201, summary_db_obj.response_text
            ...
            summary_db_obj = db.query(FlowchartSummaryHistory).filter(FlowchartSummaryHistory.url == body).order_by(
                FlowchartSummaryHistory.id.desc()).first()
            if summary_db_obj is not None:
                if summary_db_obj.flowchart is not None and summary_db_obj.flowchart != "":
        ...
        if response.status_code != 200 and response.status_code != 201:
            record_audit(data={"data": response.json(), "status_code": response.status_code},
                         event_type=f'Inside generate_summary_via_url_service Method-- Failed', log_type='error', db=db,
                         request=request)
        ...
        record_audit(data={"data": response.json(), "status_code": response.status_code},
                     event_type=f'Inside generate_summary_via_url_service Method-- Success', log_type='info', db=db,
                     request=request)
        return response.status_code, response.json().get("candidates")[0]["content"]["parts"][0]["text"]

    except Exception as err:
        record_audit(data={}, event_type=f'Inside generate_summary_via_url_service Method-- Exception',
                     log_type='error', db=db, request=request)
        raise err
...
async def create_db_entry_for_url(url, response_text, summary_type, request: Request, db: Session = Depends(get_db)):
    try:
        ...
        summary_db_obj = db.query(UrlSummaryHistory).filter(UrlSummaryHistory.url == url,
                                                            UrlSummaryHistory.summary_type == summary_type).order_by(
            UrlSummaryHistory.id.desc()).first()
        ...
        if summary_db_obj is not None:
            ...
            elif summary_db_obj.response_text is not None and summary_db_obj.response_text != "":
...
async def create_db_entry_for_flowchart(url, response_text, request: Request, db: Session = Depends(get_db)):
    try:
        ...
        summary_db_obj = db.query(FlowchartSummaryHistory).filter(FlowchartSummaryHistory.url == url).order_by(
            FlowchartSummaryHistory.id.desc()).first()

        if summary_db_obj is not None:

            if summary_db_obj.flowchart == "" or summary_db_obj.flowchart is None:
                summary_db_obj.flowchart = response_text
                db.commit()
                return response_text

            elif summary_db_obj.flowchart is not None and summary_db_obj.flowchart != "":
...
async def search_about(text, request: Request, db: Session = Depends(get_db)):
    try:
        ...
        if response.status_code != 200 and response.status_code != 201:
            record_audit(data={"data": response.json(), "status_code": response.status_code},
                         event_type=f'Inside search_about Method-- Failed', log_type='error', db=db, request=request)
            raise HTTPException(status_code=response.status_code, detail=response.text)

        record_audit(data={"data": response.json(), "status_code": response.status_code},
                     event_type=f'Inside search_about Method-- Success', log_type='info', db=db, request=request)
        return response.status_code, response.json().get("candidates")[0]["content"]["parts"][0]["text"]

    except Exception as err:
        record_audit(data={}, event_type=f'Inside search_about Method-- Exception', log_type='error', db=db,
                     request=request)
        raise err
