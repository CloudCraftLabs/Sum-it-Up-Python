import os
from src.config.data.params_local import params
...
def send_mail(sender="", recipients=["karankapoor0062@gmail.com"], subject=None, dev_details=None, cc=[], bcc=[], content_type= "text"):
    try:
        if subject is  None:
            subject = "Issue Tracker Mail"

        env = get_env_name().lower()
        if env in ['dev', 'beta', 'prod', 'local']:
...
class AWSSendEmailService:
    ...
    def aws_send_mail(sender="", recipients=[], subject="", content="", cc=[], bcc=[], content_type="text"):
        try:
            ...
            AWS_REGION_SES = params.get('AWSRegion')
